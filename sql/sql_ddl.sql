...
CREATE TABLE StellarClass (Class TEXT PRIMARY KEY, TemperatureRange TEXT, Colour TEXT);
CREATE TABLE Galaxy(Name TEXT PRIMARY KEY, Age NUMERIC, Size_T NUMERIC, "Distance from milky way" NUMERIC);
CREATE TABLE Star_BelongsTo (Name TEXT PRIMARY KEY, GalaxyName TEXT NOT NULL, Radius NUMERIC, Mass NUMERIC, StellarClassClass TEXT, FOREIGN KEY (GalaxyName) REFERENCES Galaxy(Name) ON DELETE CASCADE, FOREIGN KEY (StellarClassClass) REFERENCES StellarClass(Class) ON DELETE CASCADE);
CREATE TABLE SpaceAgency (Name TEXT PRIMARY KEY, Acronym TEXT, Region TEXT);
CREATE TABLE SpaceProgram (Name TEXT PRIMARY KEY, Objective TEXT);
CREATE TABLE Observatory (SpaceProgramName TEXT PRIMARY KEY, Location TEXT, FOREIGN KEY (SpaceProgramName) REFERENCES SpaceProgram(Name) ON DELETE CASCADE);
CREATE TABLE Mission (SpaceProgramName TEXT PRIMARY KEY, LaunchYear INTEGER, Status TEXT, FOREIGN KEY (SpaceProgramName) REFERENCES SpaceProgram(Name) ON DELETE CASCADE);
CREATE TABLE Publication (ID INTEGER PRIMARY KEY, Title TEXT NOT NULL, PeerReviewed NUMERIC, Citation TEXT );
CREATE TABLE JournalArticle (PublicationID INTEGER PRIMARY KEY, DOI TEXT, FOREIGN KEY (PublicationID) REFERENCES Publication(ID) ON DELETE CASCADE);
CREATE TABLE ConferenceProceeding (PublicationID INTEGER PRIMARY KEY, Location TEXT, FOREIGN KEY (PublicationID) REFERENCES Publication(ID) ON DELETE CASCADE);
CREATE TABLE BookChapter (PublicationID INTEGER PRIMARY KEY, BookName TEXT, FOREIGN KEY (PublicationID) REFERENCES Publication(ID) ON DELETE CASCADE);
CREATE TABLE ExoplanetDimensions (Radius NUMERIC, Mass NUMERIC, Density NUMERIC, Volume NUMERIC, PRIMARY KEY (Radius, Mass));
CREATE TABLE Researcher_WorksAt (ID TEXT PRIMARY KEY, Name TEXT, Affiliation TEXT, EmailAddress TEXT , SpaceAgencyName TEXT, FOREIGN KEY (SpaceAgencyName) REFERENCES SpaceAgency(Name) ON DELETE CASCADE);
CREATE TABLE InitiatedBy (SpaceAgencyName TEXT, SpaceProgramName TEXT, PRIMARY KEY (SpaceAgencyName, SpaceProgramName), FOREIGN KEY (SpaceAgencyName) REFERENCES SpaceAgency(Name) ON DELETE CASCADE, FOREIGN KEY (SpaceProgramName) REFERENCES SpaceProgram(Name) ON DELETE CASCADE);
CREATE TABLE WrittenBy (PublicationID INTEGER, ResearcherID TEXT, PRIMARY KEY (PublicationID, ResearcherID), FOREIGN KEY (PublicationID) REFERENCES Publication(ID) ON DELETE CASCADE, FOREIGN KEY (ResearcherID) REFERENCES Researcher_WorksAt(ID) ON DELETE CASCADE);
CREATE TABLE Exoplanet_DiscoveredAt (Name TEXT PRIMARY KEY, Type TEXT, Mass NUMERIC, Radius NUMERIC, "Discovery Year" INTEGER, "Light Years from Earth" NUMERIC, "Orbital Period" NUMERIC, Eccentricity NUMERIC, SpaceAgencyName TEXT, "Discovery Method" TEXT, FOREIGN KEY (SpaceAgencyName) REFERENCES SpaceAgency(Name) ON DELETE CASCADE, FOREIGN KEY (Mass, Radius) REFERENCES ExoplanetDimensions(Mass, Radius) ON DELETE CASCADE);
CREATE TABLE DiscoveredBy (ResearcherID TEXT, ExoplanetName TEXT, PRIMARY KEY (ResearcherID, ExoplanetName), FOREIGN KEY (ResearcherID) REFERENCES Researcher_WorksAt(ID) ON DELETE CASCADE, FOREIGN KEY (ExoplanetName) REFERENCES Exoplanet_DiscoveredAt(Name) ON DELETE CASCADE);
CREATE TABLE Orbits (ExoplanetName TEXT, StarName TEXT, PRIMARY KEY (ExoplanetName, StarName), FOREIGN KEY (ExoplanetName) REFERENCES Exoplanet_DiscoveredAt(Name) ON DELETE CASCADE, FOREIGN KEY (StarName) REFERENCES Star_BelongsTo(Name) ON DELETE CASCADE);
CREATE TABLE WrittenIn (PublicationID INTEGER, ExoplanetName TEXT, PRIMARY KEY (PublicationID, ExoplanetName), FOREIGN KEY (PublicationID) REFERENCES Publication(ID) ON DELETE CASCADE, FOREIGN KEY (ExoplanetName) REFERENCES Exoplanet_DiscoveredAt(Name) ON DELETE CASCADE);
